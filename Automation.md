...
![IPv6 Fix 1](docs/screenshots/python%20automation-%20install%20python3%20ipv6%20issue%20fix1.png)
![IPv6 Fix 2](docs/screenshots/python%20automation-%20install%20python3%20ipv6%20issue%20fix2.png)
...
![Flask Install Error 1](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error1.png)
![Flask Install Error 2](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error2.png)
...
![Flask Install Error 3](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error3.png)
![Venv Isolation 4](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error4%20venv%20isolation.png)
![Venv Isolation 5](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error5%20venv%20isolation.png)
![Flask Installed in Venv](docs/screenshots/python%20automation-%20install%20python3%20flask%20install%20error6%20venv%20isolation%20flask%20installed%20successfuly%20in%20venv.png)
...
The final `app.py` integrates all components into a single, clean automation engine.
...
THEHIVE_API_KEY = os.getenv("THEHIVE_API_KEY", "YOUR_HIVE_API_KEY_HERE")
...
def wazuh_alert():
    ...
    vt_result = check_virustotal(sha256_value)
    ...
    payload = {
        "title":       title,
        "description": json.dumps(alert, indent=4) + f"\n\nVirusTotal Result: {vt_result}",
        "type":        "internal",
        "source":      "Wazuh",
        "sourceRef":   alert["all_fields"]["rule"]["id"],
    ...
    response = requests.post(THEHIVE_URL, headers=headers, json=payload)
    print("=== TheHive response ===")
    print(response.text)

    # Build and send email alert
    sha256_line = f"SHA256: {sha256_value}" if sha256_value else "SHA256: Not found"
    vt_link     = f"https://www.virustotal.com/gui/file/{sha256_value}" if sha256_value else "N/A"
    email_body  = f"""Alert Title: {title}
Severity: {severity_label}
Agent: {agent_name}
{sha256_line}
VirusTotal Link: {vt_link}
MITRE IDs: {', '.join(mitre_ids) if mitre_ids else 'N/A'}
...
"""
    send_email_alert(title, email_body)
